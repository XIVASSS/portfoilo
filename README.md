...
Personal portfolio built with **Next.js (App Router)**, **TypeScript**, and **Tailwind CSS**. The home layers a hero flip mosaic under a handwritten headline, then a **scrapbook grid** of projects (wide tiles for LinkedIn-ready **post** artwork). Cards tilt on hover, straighten when focused, and expose chips for **Instructables / LinkedIn feed / GitHub** where configured in [`src/content/projects.ts`](src/content/projects.ts). [`public/vibe-collage.png`](public/vibe-collage.png) backs the mood-board strip (`#vibe`). Stick-figure silhouettes stroll along the bottom edge (`WalkingCharacters`).
